...
function testReducer(sampleState: number) {
	return (params: TAutomataEvent<TTestState, TTestAction, TTestContext<TTestState>, TTestPayload<TTestAction>>) => ({
		context: {
...
describe(`automata`, () => {
	...
	describe('constructor', () => {
		it('returns an instance of AutomataTest', () => {
	...
	describe('/setStateValidator', () => {
		...
		it('resets the State Validator to default when called with null', () => {
			sampleInstance.setStateValidator(testValidator);
	...
	describe('when not initialized properly', () => {
		it('throws an error when calling `dispatch`', () => {
			expect(() =>
	...
	describe('/init', () => {
		...
		it('throws when called with non-function reducer', () => {
			expect(() => {
				sampleInstance.init({
		...
		it('assigns a reducer when passed', () => {
			const sampleState = sampleRange(0, 100);
		...
		it('initializes with provided Context, empty Queue, and is Enabled and Resumed by default', () => {
			const sampleState = sampleRange(0, 100);
			const sampleContext = null;
			sampleInstance.init({
				state: sampleState,
		...
		it('resets the Queue and Internal State, while copying Disabled and Paused, when invoked upon stateful Automata', () => {
			const sampleState = sampleRange(0, 100);
		...
		it('throws when called with invalid state', () => {
			expect(() => {
				sampleInstance.init({
					state: null,
					context: null,
	...
	describe('/consumeAction', () => {
		...
		it('with empty Action Queue returns the current Context ', () => {
		...
		it('with empty Action Queue returns the current Context when called with parameter ', () => {
			sampleInstance.clearActionQueue();
			const reducer = sampleInstance.getReducer();
			if (!reducer)
				throw new Error('No Reducer');
		...
		it('when Disabled, doesn\'t alter the Action Queue but returns computed Context', () => {
			sampleInstance.disable();
			const reducer = sampleInstance.getReducer();
			const result = sampleInstance.consumeAction();
			if (!reducer)
		...
		it('when Enabled, pops the Action Queue and returns computed Context', () => {
			const newAction = {
		...
		describe('with argument', () => {
			...
			it('when Disabled, computes the specified quantity of Actions from the Queue without changing the internal state', () => {
			...
			it('when Enabled, consumes of Actions from the Queue and changes the state', () => {
				sampleInstance.enable();
				sampleInstance.clearActionQueue();
				const reducer = sampleInstance.getReducer();
				if (!reducer)
	...
	describe('/dispatch', () => {
		...
		describe('when Unpaused, Enabled', () => {
			...
			it('instantly updates internal Context without the Action Queue and returns new Context', () => {
		...
		describe('when Unpaused, Disabled', () => {
			...
			it('returns new Context without altering Action Queue or changing internal Context', () => {
				const result = sampleInstance.dispatch(sampleAction);
				const reducer = sampleInstance.getReducer();
				if (!reducer) {
					throw new Error('reducer is null');
		...
		describe('when Paused, Enabled', () => {
			...
			it('returns new Context, adding Action to Action Queue and not changing internal Context', () => {
				const result = sampleInstance.dispatch(sampleAction);
				const reducer = sampleInstance.getReducer();
				if (!reducer) {
					throw new Error('reducer is null');
	...
	describe('/pause, /resume, /disable, /enable', () => {
		...
		describe('when Paused', () => {
			...
			it('resumes the Disabled Automata, preserving the queue', () => {
				sampleInstance.disable();
				sampleInstance.resume();
				expect(sampleInstance.isPaused()).toBe(false);
				expect(sampleInstance.isEnabled()).toBe(false);
			...
			it('resumes the Enabled Automata, Consuming the queue', () => {
				sampleInstance.enable();
				sampleInstance.resume();
				expect(sampleInstance.isPaused()).toBe(false);
				expect(sampleInstance.isEnabled()).toBe(true);
			...
			it('does not Dispatch the Action but queues it', () => {
				sampleInstance.dispatch(sampleAction);
			...
			describe('enabling/Disabling in Paused mode', () => {
				it('disable is invariant to Paused and Context, but not to Action Queue', () => {
					sampleInstance.disable();
					sampleInstance.dispatch(sampleAction);
					expect(sampleInstance.getActionQueue()).toEqual([...aQueue, sampleAction]);
					expect(state).toMatchObject(sampleInstance.getContext());
				...
				it('enable is invariant to Paused, and Context, but not to Action Queue', () => {
					sampleInstance.enable();
					sampleInstance.dispatch(sampleAction);
					expect(sampleInstance.getActionQueue()).toEqual([...aQueue, sampleAction]);
					expect(state).toMatchObject(sampleInstance.getContext());
				...
				it('disable(true) clears the Action Queue but invariant to Context and Paused', () => {
					sampleInstance.disable(true);
					sampleInstance.dispatch(sampleAction);
					expect(state).toMatchObject(sampleInstance.getContext());
		...
		describe('when not Paused', () => {
			...
			it('pauses the enabled Automata', () => {
				expect(sampleInstance.getActionQueue()?.length).toBe(0);
				sampleInstance.enable();
				sampleInstance.pause();
				expect(sampleInstance.getActionQueue()?.length).toBe(0);
			...
			it('pauses the disabled Automata', () => {
				expect(sampleInstance.getActionQueue()?.length).toBe(0);
				sampleInstance.disable();
				sampleInstance.pause();
				expect(sampleInstance.getActionQueue()?.length).toBe(0);
			...
			describe('enabling/Disabling not in Paused mode', () => {
				it('disable is invariant to Action Queue, Context and Paused', () => {
					sampleInstance.disable();
					sampleInstance.dispatch(sampleAction);
					expect(sampleInstance.getActionQueue()).toEqual(aQueue);
					expect(state).toMatchObject(sampleInstance.getContext());
				...
				it('disable with \'true\' is invariant to Action Queue, Context and Paused', () => {
					sampleInstance.disable(true);
					sampleInstance.dispatch(sampleAction);
					expect(sampleInstance.getActionQueue()).toEqual(aQueue);
					expect(state).toMatchObject(sampleInstance.getContext());
				...
				it('enable is invariant to Paused and Action Queue, but not to Context', () => {
					sampleInstance.enable();
					sampleInstance.dispatch(sampleAction);
					expect(sampleInstance.getActionQueue()).toEqual(aQueue);
					const reducer = sampleInstance.getReducer();
	...
	describe('/getActionQueue', () => {
		...
		it('returns the Action Queue when Paused', () => {
			const action = {
				action: sampleRange(1, 5),
		...
		it('is always empty when Resumed and Enabled', () => {
			sampleInstance.resume();
			sampleInstance.enable();
			expect(sampleInstance.getActionQueue()).toEqual([]);
			const action = {
		...
		it('every call returns a new object', () => {
			const action = {
				action: sampleRange(1, 5),
				payload: { value: sampleRange(100, 1000) },
			};
	...
	describe('/getContext', () => {
		...
		it('returns the Context passed at initialization', () => {
	...
	describe('/getReducer', () => {
		...
		it('returns the RootReducer passed at initialization', () => {
		...
		it('returns Null when RootReducer is reset', () => {
			sampleInstance.init({
				rootReducer: null,
				...sampleState,
			});
	...
	describe('/clearActionQueue', () => {
		...
		it('clears the Action Queue when Paused, Enabled', () => {
			sampleInstance.pause();
			sampleInstance.enable();
			sampleInstance.dispatch(sampleAction);
			expect(sampleInstance.getActionQueue()).toEqual([sampleAction]);
		...
		it('clears the Action Queue when Paused, Disabled', () => {
			sampleInstance.pause();
			sampleInstance.disable();
			sampleInstance.dispatch(sampleAction);
			expect(sampleInstance.getActionQueue()).toEqual([sampleAction]);
		...
		it('isn\'t needed when Resumed, Enabled', () => {
			sampleInstance.resume();
			sampleInstance.enable();
			sampleInstance.dispatch(sampleAction);
			expect(sampleInstance.getActionQueue()).toEqual([]);
		...
		it('isn\'t needed when Resumed, Disabled', () => {
			sampleInstance.resume();
			sampleInstance.disable();
			sampleInstance.dispatch(sampleAction);
			expect(sampleInstance.getActionQueue()).toEqual([]);
	...
	describe('/collapseActionQueue', () => {
		...
		describe('when Paused', () => {
			...
			it('when Enabled, clears the Action Queue and returns it along with new Context', () => {
				const reducer = sampleInstance.getReducer();
				if (!reducer) {
					throw new Error('No Reducer');
				}
			...
			it('when Disabled, returns the Action Queue and Context, but doesn\'t change them', () => {
				sampleInstance.disable(true);
				const reducer = sampleInstance.getReducer();
				if (!reducer) {
					throw new Error('No Reducer');
		...
		describe('when Resumed', () => {
			...
			it('when Enabled, does not change anything, returns the current Context and empty Actions', () => {
				sampleInstance.enable();
				const reducer = sampleInstance.getReducer();
				if (!reducer) {
					throw new Error('No Reducer');
