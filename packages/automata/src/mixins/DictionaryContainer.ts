 ...
 * split by namespaces
 */
import { TAbstractConstructor, TMixin } from '../types/index.js';
...
>(
	...
	return class DictionaryContainer extends base {
		...
		_getItemValue(itemKey: string, namespace?: string) {
			...
			if (Object.values(this._dictionary).includes(value as ItemType))
				value =
					value + this._stringHash(JSON.stringify(this._dictionary));
...
export interface DictionaryContainer
	extends TMixin<typeof DictionaryContainer> {}

export const DictionaryContainerFactory = <ItemType>(
	base: typeof AbstractBaseClass = AbstractBaseClass
): TAbstractConstructor<DictionaryContainer> =>
	DictionaryContainer<ItemType, typeof base>(base);

export class GenericDictionaryContainer extends DictionaryContainerFactory() {}
