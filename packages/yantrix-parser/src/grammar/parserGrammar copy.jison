...
[0-9]+("."[0-9]+)?e\+[0-9]+          {this.popState(); return 'expoLiteral'}
...
'effect/'                             {this.begin('EffectStatement'); return 'effect/'}
<EffectStatement>[^()=<\n\s]+         {this.popState(); return 'EffectName'}
...
ContextDefinitions
        ...
        | ContextStatement '<=' '(' KeyList')' {if($4.length > $1.length) {throw new Error(`The number of payload arguments must be equal to or less than the context argument`);}$$ = {context:$1, payload:$4}}
...
ContextStatement
        : '#{' KeyList '}'  } {$$ = $2};
EffectStatement
        : 'effect/' EffectName {$$ = {effectName: $2}}
        | 'effect/' EffectName '<=' '(' KeyList  ')' {$$ = {
           effectName:$2,
           payload: $5
          }
        }
...
KeyList  
        ...
        ;
KeyItem  
        : TargetProperty '=' Expression {if($3.hasOwnProperty('Property')){if($3['Property'] === $1){throw new Error('The property cannot match the target property')}};$$ = {KeyItemDeclaration: {
...
Expression
          ...
          | Constant
          | expoLiteral {$$ = {ExponentialValue: Number($1)}}
