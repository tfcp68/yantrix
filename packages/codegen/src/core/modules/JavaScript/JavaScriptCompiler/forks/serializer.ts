...
*/
import { TActionChain } from '@yantrix/mermaid-parser';
...
function processStateActions(props: {
	state: string;
	diagram: TStateDiagramMatrixIncludeNotes;
	actionDictionary: BasicActionDictionary;
	stateDictionary: BasicStateDictionary;
	...
	const possibleActions = props.diagram.actionChains[props.state]!;
	for (const action in possibleActions) {
		...
		const processedStateChecks = processActionChains({
			chains,
...
function processActionChains(props: {
	chains: TActionChain[];
	stateDictionary: BasicStateDictionary;
	expressionRecord: TExpressionRecord;
}) {
	...
	for (const { chain, state: transitionState } of props.chains) {
		const transitionStateId = props.stateDictionary.getDictionary()[transitionState];
		const conditions = processChainSegments({ chain, expressionRecord: props.expressionRecord });
	...
	return processedStateChecks;
}

function processChainSegments(props: {
	chain: string[];
	expressionRecord: TExpressionRecord;
}) {
	/*
	...
	*/
	...
	const conditions = [];
	for (const segment of props.chain) {
		try {
