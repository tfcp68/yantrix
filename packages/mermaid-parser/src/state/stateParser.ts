...
function getTransitions(parsedDiagram: TParsedDiagramArray): TTransitionsArray {
  ...
  for (let i = 0; i < parsedDiagram.length; i++) {
    if (parsedDiagram[i].stmt === 'relation') {
      ...
      const tempSt1: Record<string, string> = parsedDiagram[i].state1 as Record<
        string,
        string
      >;
      const tempSt2: Record<string, string> = parsedDiagram[i].state2 as Record<
        string,
        string
      >;
    ...
    if (parsedDiagram[i].stmt === 'state') {
      ...
      if (keys.includes('note')) {
        ...
        if (noteKeys.includes('text')) {
          ...
          const parsedDiagramNote: Record<string, string> = parsedDiagram[i]
            .note as Record<string, string>;
