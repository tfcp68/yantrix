...
>({
  routes: {
    [WeatherEvents.FETCH_WEATHER]: [
      ...
      (event: TAutomataEventMetaType<WeatherEvents.FETCH_WEATHER, WeatherMeta>) => {
      ...
      (response: any): TAutomataEventStack<WeatherEvents, WeatherMeta> => {
...
(async () => {
    weatherSource.addListener("weather-listener", (data) => {
        console.log(data)
        return [
            {
                event: WeatherEvents.WEATHER_RECEIVED,
