...
| `_currentEpoch`  | () => **Number**   | module | Global counter incremented on each successful transition across all FSM instances in the same generated module; useful as a discrete timer |
| `_currentCycle`  | () => **Number**   | `FSM` | Per-instance transition counter, starting at 1 and incremented on each successful dispatch |
| `_currentTimestamp`  | () => **Number**   | any | Current system time in microseconds |
| `_currentTime`  | () => **String**   | any | Current date and time in ISO-8601 format |
