...
functionDictionary.register(builtInFunctions);
  ...
  "Switch": "Switch"
			...
			export type TActionsAutomata = keyof typeof actionsMap;
			const statesMap = {
  "~~~START~~~": "~~~START~~~",
  "Off": "Off",
  "Red": "Red",
...
export class Automata extends GenericAutomata {

    static id = 'Automata';
    static actions = actionsMap;
    static states = statesMap;
    static getState = (state: keyof typeof statesMap) => statesDictionary[state];
    static hasState = (instance: Automata, state: keyof typeof Automata.states) => instance.state === Automata.getState(state);
    ...
    static createAction = (action: keyof typeof actionsMap, payload:any) => {
		const actionId = Automata.getAction(action);
		return {
			action: actionId,
			payload,
		}
